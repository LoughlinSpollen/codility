...
# ...
# A[N − K], A[N − K + 1], ..., A[N − 1].
# What's more, every block should contain at least one peak. Notice that extreme 
# elements of the blocks (for example A[K − 1] or A[K]) can also be peaks, but only if 
# they have both neighbors (including one in an adjacent blocks).
...
# that, given a non-empty array A consisting of N integers, returns the maximum number 
# of blocks into which A can be divided.
...
def solution(A):
    ...
    for i in range (1, sz-1):
        currentVal = intArr[i]
        nextVal = intArr[i+1]
        if currentVal > prevVal and currentVal > nextVal:
            peaks.append(i)
        currentVal = nextVal
...
class TestPeaks(unittest.TestCase):
    ...
    def test_min(self):
        self.assertEqual(solution([1, 2, 1, 1, 2, 1, 1, 2, 1, 1, 2, 1, 1, 2, 1, 1, 2, 1, 1, 2, 1, 1, 2, 1, 1, 2, 1, 1, 2, 1]), 10)
        self.assertEqual(solution([1, 2, 3, 4, 6, 5]), 1)

    def test_max(self):
        self.assertEqual(solution([1000000000]*100000), 0)