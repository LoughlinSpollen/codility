...
# on day P and sold on day Q, where 0 ≤ P ≤ Q < N, then the profit of such transaction 
# is equal to A[Q] − A[P], provided that A[Q] ≥ A[P]. Otherwise, the transaction 
...
# because A[2] − A[0] = 21123 − 23171 = −2048. If a share was bought on day 4 and sold 
# on day 5, a profit of 354 would occur because A[5] − A[4] = 21367 − 21013 = 354. 
...
def solution(A):
    ...
    if size > 0:
        bestBuyPrice = intArr[0]
   
        for i in range (1, size):                
            sellPrice = intArr[i]
            bestProfit = max(sellPrice - bestBuyPrice, bestProfit)                                
            buyPrice = intArr[i]
            bestBuyPrice = min(bestBuyPrice, buyPrice)
...
class TestMaxProfit(unittest.TestCase):
    ...
    def test_max(self):
        self.assertEqual(solution([200000,200000,200000,200000,200000,200000]), 0)
