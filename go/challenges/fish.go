...
// All its elements are unique. Array B contains the directions of the fish. It contains
...
// the smaller one. More precisely, we say that two fish P and Q meet each other when
// P < Q, B[P] = 1 and B[Q] = 0, and there are no living fish between them. After they meet:

// If A[P] > A[Q] then P eats Q, and P will still be flowing downstream,
// If A[Q] > A[P] then Q eats P, and Q will still be flowing upstream.
// We assume that all the fish are flowing at the same speed.
// That is, fish moving in the same direction never meet. The goal is to calculate the number
...
func SolutionFish(A []int, B []int) int {
	fishArr := A
	direction := B
	const downStream = 0
	...
	count := 0
	upstreamFish := []int{}
	for i := 0; i < len(fishArr); i++ {
		fish := fishArr[i]
		if direction[i] == downStream {
			for len(upstreamFish) != 0 {
				if upstreamFish[len(upstreamFish)-1] > fish {
					count += 1
					break
				} else {
					upstreamFish = upstreamFish[:len(upstreamFish)-1]
					count -= 1
				}
			}
		} else {
			upstreamFish = append(upstreamFish, fish)
			count += 1
		}
	}
	return count
