...
func SolutionGenomicRangeQuery(S string, P []int, Q []int) []int {
	DNAsequence := S
	queriesStart := P
	queriesEnd := Q

	factors := []int{}
	for i := 0; i < len(queriesStart); i++ {
		start := queriesStart[i]
		end := queriesEnd[i] + 1
		impactFactor := -1
		for j := start; j < end; j++ {
			if DNAsequence[j] == 'A' {
				impactFactor = 1
			} else if DNAsequence[j] == 'C' {
				if impactFactor > 1 {
					impactFactor = 2
				}
			} else if DNAsequence[j] == 'G' {
				if impactFactor < 3 {
					impactFactor = 3
				}
			} else if DNAsequence[j] == 'T' {
				if impactFactor < 4 {
					impactFactor = 4
				}
			}
